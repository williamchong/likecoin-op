...
// QueryNftClasses chains the current query on the "nft_classes" edge.
func (_q *AccountQuery) QueryNftClasses() *NFTClassQuery {
...
func (_q *AccountQuery) QueryStakingEvents() *StakingEventQuery {
...
func (_q *AccountQuery) QueryStakings() *StakingQuery {
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withNftClasses:    _q.withNftClasses.Clone(),
...
// WithNftClasses tells the query-builder to eager-load the nodes that are connected to
// the "nft_classes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithNftClasses(opts ...func(*NFTClassQuery)) *AccountQuery {
	query := (&NFTClassClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withNftClasses = query
	return _q
}
...
func (_q *AccountQuery) WithStakingEvents(opts ...func(*StakingEventQuery)) *AccountQuery {
	query := (&StakingEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) WithStakings(opts ...func(*StakingQuery)) *AccountQuery {
	query := (&StakingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(account.FieldEvmAddress).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Account.Query().
//		Select(account.FieldEvmAddress).
//		Scan(ctx, &v)
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withNftClasses; query != nil {
		if err := _q.loadNftClasses(ctx, query, nodes,
			func(n *Account) { n.Edges.NftClasses = []*NFTClass{} },
			func(n *Account, e *NFTClass) { n.Edges.NftClasses = append(n.Edges.NftClasses, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withStakingEvents; query != nil {
		if err := _q.loadStakingEvents(ctx, query, nodes,
			func(n *Account) { n.Edges.StakingEvents = []*StakingEvent{} },
			func(n *Account, e *StakingEvent) { n.Edges.StakingEvents = append(n.Edges.StakingEvents, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withStakings; query != nil {
		if err := _q.loadStakings(ctx, query, nodes,
			func(n *Account) { n.Edges.Stakings = []*Staking{} },
			func(n *Account, e *Staking) { n.Edges.Stakings = append(n.Edges.Stakings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AccountQuery) loadNftClasses(ctx context.Context, query *NFTClassQuery, nodes []*Account, init func(*Account), assign func(*Account, *NFTClass)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.NftClassesTable)
		s.Join(joinT).On(s.C(nftclass.FieldID), joinT.C(account.NftClassesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.NftClassesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.NftClassesPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*NFTClass](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "nft_classes" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *AccountQuery) loadStakingEvents(ctx context.Context, query *StakingEventQuery, nodes []*Account, init func(*Account), assign func(*Account, *StakingEvent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(stakingevent.FieldAccountID)
	}
	query.Where(predicate.StakingEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.StakingEventsColumn), fks...))
	}))
...
func (_q *AccountQuery) loadStakings(ctx context.Context, query *StakingQuery, nodes []*Account, init func(*Account), assign func(*Account, *Staking)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(staking.FieldAccountID)
	}
	query.Where(predicate.Staking(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.StakingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
