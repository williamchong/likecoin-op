...
async function createNFTClassFromJSON(
  iscnId,
  signingClient,
  account,
  { nftMaxSupply } = {},
  ...
  console.log(
    'Creating NFT Class - Reading data from ./data/nft_class.json...',
  );
  ...
  const classId = ((attribute && attribute.value) || '').replace(
    /^"(.*)"$/,
    '$1',
  );
...
async function createRoyaltyConfig(classId, iscnId, signingClient, account) {
  try {
    ...
    await signingClient.createRoyaltyConfig(account.address, classId, {
      rateBasisPoints,
      stakeholders,
    });
  } catch (err) {
...
async function mintNFTsFromJSON(classId, nftCount, signingClient, account) {
  ...
  const content = readFileSync(
    path.join(__dirname, './data/nfts_default.json'),
  );
  const defaultData = JSON.parse(content);
  if (!defaultData || defaultData.uri === undefined)
    throw new Error('Invalid NFT data json');
  ...
  if (listData.length && listData.length !== nftCount)
...
async function run() {
  ...
  const args = yargsParser(process.argv.slice(2), {
    boolean: ['create-new-iscn', 'help'],
    string: ['iscn-id', 'class-id'],
    number: ['nft-count', 'nft-max-supply'],
  });
  const { createNewIscn, nftCount, nftMaxSupply, help } = args;
  let { iscnId, classId } = args;
  if (help) {
    printHelp();
    return;
  }
  ...
  if (createNewIscn && (iscnId || classId)) {
    console.error(
      'Cannot create new ISCN and use existing ISCN ID or class ID at the same time',
    );
    printHelp();
    return;
  }
  if (!createNewIscn && !iscnId && !classId) {
    console.error(
      'Either --create-new-iscn, --iscn-id, --class-id must be set',
    );
    printHelp();
    return;
  }
  ...
  try {
    ...
    if (!classId) {
      classId = await createNFTClassFromJSON(iscnId, signingClient, account, {
        nftMaxSupply,
      });
    ...
    if (
      ...
      console.error(
        `The wallet does not own the ISCN ID ${iscnId} or NFT Class ID ${classId}`,
      );
    } else {
      console.error(error);
    }
