...
func (aq *AccountQuery) QueryNftClasses() *NFTClassQuery {
	query := (&NFTClassClient{config: aq.config}).Query()
...
func (aq *AccountQuery) QueryNfts() *NFTQuery {
	query := (&NFTClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withNftClasses: aq.withNftClasses.Clone(),
		withNfts:       aq.withNfts.Clone(),
...
func (aq *AccountQuery) WithNftClasses(opts ...func(*NFTClassQuery)) *AccountQuery {
	query := (&NFTClassClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withNftClasses = query
	return aq
}
...
func (aq *AccountQuery) WithNfts(opts ...func(*NFTQuery)) *AccountQuery {
	query := (&NFTClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(account.FieldCosmosAddress).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Account.Query().
//		Select(account.FieldCosmosAddress).
//		Scan(ctx, &v)
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withNftClasses; query != nil {
		if err := aq.loadNftClasses(ctx, query, nodes,
	...
	if query := aq.withNfts; query != nil {
		if err := aq.loadNfts(ctx, query, nodes,
			func(n *Account) { n.Edges.Nfts = []*NFT{} },
			func(n *Account, e *NFT) { n.Edges.Nfts = append(n.Edges.Nfts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadNftClasses(ctx context.Context, query *NFTClassQuery, nodes []*Account, init func(*Account), assign func(*Account, *NFTClass)) error {
	...
	query.withFKs = true
	query.Where(predicate.NFTClass(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.NftClassesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_nft_classes
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_nft_classes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_nft_classes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AccountQuery) loadNfts(ctx context.Context, query *NFTQuery, nodes []*Account, init func(*Account), assign func(*Account, *NFT)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.NFT(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.NftsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_nfts
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_nfts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_nfts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
